...
$cc.validate = function(e){
  ...
  if (e.target.value == ''){
    e.target.previousElementSibling.className = 'input-container';
    e.target.nextElementSibling.className = 'input';
    return
  }
  ...
  if (isLuhn == true){
    e.target.nextElementSibling.className = 'input-container'
  }else{
    e.target.nextElementSibling.className = 'input'
  }
  ...
  for (var i = 0; i< card_types.length; i++){
    if (number.match(card_types[i].pattern)){
      ...
      e.target.previousElementSibling.className = 'input '+card_types[i].name;
    }
  }
}
